...
[TrueBlocks](https://github.com/TrueBlocks/trueblocks-core) is a tool for managing optimized local indices of EVM chain data. TrueBlocks then makes these local data copies accessible through an enhanced RPC interface. TrueBlocks delivers some of the highest performance ways to query chain data and it excels at tracing and querying all appearances of a given address throughout a chain's history. Since TrueBlocks can provide its data over RPC, it could be used as an ultra high performance RPC provider for `ctc`.
